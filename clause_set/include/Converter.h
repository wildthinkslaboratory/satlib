...
///////////////////////////////////////////
/*
  The Converter class takes an InputTheory and then using the member function
  translateTo you can convert it to an actual ClauseSet of the type that you
  desire. We avoid a switch statement on the desired type by making dummy type
  classes and using double dispatch. I'm not entirely convinced of the value of
  this software engineering choice.  It looks very nice in the main call.  The
  converter takes an input and desired type and gives us the clauseSet we want
  in a single line.  It feels easier to maintain than a bunch of case
  statements.  Still might be overkill.
*/
...
class PfsConverter : public Converter {
  ...
  vector<vector<size_t> > get_assignments(const vector<string>& var_set,
                                          size_t domain_size);
...
inline ClauseSet* PfsConverter::convert(ClauseSetType t) {
  switch (t) {
    case CNF:
      return convert_to_cnf();
    case SYMRES:
      return convert_to_symres();
    case PFS:
      return convert_to_pfs();
    case GROUP_BASED:
      quit("can't convert PFS to GROUP_BASED yet");
      break;
    case NOT_SPECIFIED:
      return convert_to_pfs();
    default:
      quit("can't convert PFS clause set");
  }
  return NULL;
}
...
inline ClauseSet* CnfConverter::convert(ClauseSetType t) {
  switch (t) {
    ...
    case PFS:
      return convert_to_pfs();
    case GROUP_BASED:
      quit("can't convert CNF to GROUP_BASED yet");
      break;
    case NOT_SPECIFIED:
      return convert_to_cnf();
    default:
      quit("can't convert CNF clause set");
      return NULL;
  }
  return NULL;
}
