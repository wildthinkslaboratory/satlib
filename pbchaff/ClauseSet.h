...
inline const Conflict& ClauseSet::getConflict() const
{
  switch (m_whereToGetConflict)
    {
    case LAZY : return m_lazyClauses.getConflict();
    case PB : return m_PBClauses.getConflict();
    case MOD2 : return m_mod2Clauses.getConflict();
    }
}
...
inline void ClauseSet::initializeClause(FastClause& c, ClauseID id) const
{
  ...
  switch (whichSet)
    {
    case LAZY : m_lazyClauses.initializeClause(c,id / 3); break;
    case PB : m_PBClauses.initializeClause(c,id / 3); break;
    case MOD2 : m_mod2Clauses.initializeClause(c,id /3); break;
    }
}
...
inline const ClauseSetStatistics& ClauseSet::getStatistics()
{
  ...
  m_statistics.literalCount = lazyStats.literalCount + PBStats.literalCount + mod2Stats.literalCount;
  m_statistics.outOfMemoryCount = lazyStats.outOfMemoryCount + PBStats.outOfMemoryCount
    + mod2Stats.outOfMemoryCount;
  m_statistics.initialClauseCount = lazyStats.initialClauseCount + PBStats.initialClauseCount
    + mod2Stats.initialClauseCount;
  m_statistics.addedClauseCount = lazyStats.addedClauseCount + PBStats.addedClauseCount
    + mod2Stats.addedClauseCount;
  m_statistics.deletedClauseCount = lazyStats.deletedClauseCount + PBStats.deletedClauseCount
...
inline void ClauseSet::verifyReason(Literal l, ClauseID id)
{
  ...
  switch (whichSet)
    {
    case LAZY : m_lazyClauses.verifyReason(l,id / 3); break;
    case PB : m_PBClauses.verifyReason(l,id / 3); break;
    case MOD2 : m_mod2Clauses.verifyReason(l,id /3); break;
    }
}
...
inline void ClauseSet::getWhyOverSat(FastClause& c, AtomValueMap&
				     whyLits,std::vector<int>& reasons)
{
  ASSERT(!c.isMod2());
  m_satLits.clear();
  ...
  for (std::size_t i = 0; i < m_satLits.size(); i++) {
    ...
    if (id == 0) whyLits.addAtom(atom,m_satLits.getValue(atom)) ;
    else
      {
	int whichSet = id % 3;
	...
	switch (whichSet)
	  {
	  case LAZY : m_lazyClauses.getUnfavorables(id / 3,whyLits); break ;
	  case PB : m_PBClauses.getUnfavorables(id / 3,whyLits); break;
	  case MOD2 : m_mod2Clauses.getUnfavorables(id / 3,whyLits,atom); break;
	  }
      }
  } 
}
...
inline void ClauseSet::removeClause(ClauseID id)
{
  ...
  switch (whichSet)
    {
    case LAZY : m_lazyClauses.removeClause(id / 3); break;
    case PB : m_PBClauses.removeClause(id / 3); break;
    default : fatalError("removeClause not supported in ClauseSet::removeClause");
    }
}
