...
int main(int argc, char* argv[])
{
  ...
  cout << "  // fluent predicates " << endl
       ...
       << "PREDICATE powerOn( instrument dummy time ) ;	 " << endl
       << "PREDICATE calibrated( instrument time ) ; " << endl
       << "PREDICATE haveImage( direction mode time ) ; " << endl
       << "PREDICATE next( time time ) ; " << endl << endl;
  
  cout << "FORALL( i b t2 t1 ) EXISTS(s)  ~next[ t1 t2 ] ~powerOn[i b t2] powerOn[i b t1] onBoard[i s]  { t1 != t2 } ;"  << endl
       << "FORALL( i b t2 t1 ) EXISTS(s)  ~next[ t1 t2 ] ~powerOn[i b t2] powerOn[i b t1] powerAvail[ s t1 ]	{ t1 != t2 } ;"  << endl
       ...
       << "FORALL( i b t1 t2 ) EXISTS(s)  ~next[ t1 t2 ] powerOn[ i b t2 ] ~powerOn[ i b t1 ] onBoard[ i s ] 	{ t1 != t2 } ;"  << endl
       << "FORALL( s b t2 t1 ) EXISTS(i)  ~next[ t1 t2 ] ~powerAvail[s t2] powerAvail[s t1] powerOn[ i b t1 ] 	{ t1 != t2 } ;"  << endl
    
       << "FORALL( i t1 t2 ) EXISTS( s )  ~next[ t1 t2 ] ~calibrated[ i t2 ] calibrated[ i t1 ] onBoard[ i s ] 	{ t1 != t2 } ;"  << endl
       << "FORALL( i t1 t2 ) EXISTS( s d )  ~next[ t1 t2 ] ~calibrated[ i t2 ] calibrated[ i t1 ] pointing[ s d t1 ] 	{ t1 != t2 } ;"  << endl
       << "FORALL( i b t1 t2 ) ~next[ t1 t2 ] ~calibrated[ i t2 ] calibrated[ i t1 ] powerOn[ i b t1 ] 	{ t1 != t2 } ;"  << endl
    
       << "FORALL( d m t1 t2 ) EXISTS( i )  ~next[ t1 t2 ] ~haveImage[ d m t2 ] haveImage[ d m t1 ] calibrated[ i t1 ] 	{ t1 != t2 } ;"  << endl
       << "FORALL( d m t1 t2 ) EXISTS( i s )  ~next[ t1 t2 ] ~haveImage[ d m t2 ] haveImage[ d m t1 ] onBoard[ i s ] 	{ t1 != t2 } ;"  << endl
       << "FORALL( d m t1 t2 ) EXISTS( i )  ~next[ t1 t2 ] ~haveImage[ d m t2 ] haveImage[ d m t1 ] supports[ i m ] 	{ t1 != t2 } ;"  << endl
       << "FORALL( d b m t1 t2 ) EXISTS( i )  ~next[ t1 t2 ] ~haveImage[ d m t2 ] haveImage[ d m t1 ] powerOn[ i b t1 ] 	{ t1 != t2 } ;"  << endl
       << "FORALL( d m t1 t2 ) EXISTS( s )  ~next[ t1 t2 ] ~haveImage[ d m t2 ] haveImage[ d m t1 ] pointing[ s d t1 ] 	{ t1 != t2 } ;"  << endl << endl;

    
  cout << "// change of state" << endl
       << "FORALL( i b s t1 t2 ) EXISTS ( d ) ~next[ t1 t2 ] ~powerOn[ i b t1 ] ~onBoard[ i s ] ~calibrated[ i t2 ] pointing[ s d t1 ] ;" << endl
       << "FORALL( i s b d m t1 t2 ) ~next[ t1 t2 ] ~calibrated[ i t1 ] ~onBoard[ i s ] ~supports[ i m ] ~powerOn[ i b t1 ] haveImage[ d m t1 ] ~haveImage[ d m t2 ] pointing[ s d t1 ] 	{ t1 != t2 };" << endl
       << "FORALL( i s b d m t1 t2 ) ~next[ t1 t2 ] ~calibrated[ i t1 ] ~onBoard[ i s ] ~supports[ i m ] ~powerOn[ i b t1 ] haveImage[ d m t1 ] ~haveImage[ d m t2 ] pointing[ s d t2 ] 	{ t1 != t2 };" << endl
